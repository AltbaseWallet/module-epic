...
static POW_TYPE_STRING: [&'static str; 4] = ["algo0000", "algo0001", "algo002", "algo003"];
...
static POW_TYPE_WIRE_MASK: u8 = 0x5a;
static POW_TYPE_WIRE_STRING: [&[u8]; 4] = [
    &[0x39, 0x2f, 0x39, 0x31, 0x3b, 0x28, 0x35, 0x35],
    &[0x39, 0x2f, 0x39, 0x31, 0x3b, 0x2e, 0x35, 0x35],
    &[0x28, 0x3b, 0x34, 0x3e, 0x35, 0x37, 0x22],
    &[0x2a, 0x28, 0x35, 0x3d, 0x2a, 0x35, 0x2d],
];

fn pow_type_index(value: PoWType) -> usize {
    match value {
        PoWType::Cuckaroo => 0,
        PoWType::Cuckatoo => 1,
        PoWType::RandomX => 2,
...
fn pow_type_wire_name(index: usize) -> String {
    POW_TYPE_WIRE_STRING[index]
        .iter()
        .map(|value| (*value ^ POW_TYPE_WIRE_MASK) as char)
        .collect()
}

fn pow_type_from_wire_name(value: &str) -> Option<PoWType> {
    POW_TYPE_VALUE
        .iter()
        .enumerate()
        .find_map(|(index, pow_type)| {
            if value == POW_TYPE_STRING[index] || value == pow_type_wire_name(index) {
                Some(*pow_type)
            } else {
                None
            }
...
impl Serialize for PoWType {
    ...
    where
        S: Serializer,
    {
        let name = pow_type_wire_name(pow_type_index(*self));
...
impl<'de> Deserialize<'de> for PoWType {
        ...
        D: Deserializer<'de>,
    {
        ...
        match pow_type_from_wire_name(s.as_str()) {
            Some(x) => Ok(x),
