...
impl Slate {
        ...
        K: Keychain,
    {
        ...
        #[cfg(test)]
        let message_nonce = if use_test_rng {
            Some(&test_message_nonce)
        } else {
            None
        };
        #[cfg(not(test))]
        let message_nonce: Option<&SecretKey> = {
            let _ = use_test_rng;
            None
        };
        ...
        K: Keychain,
    {
        ...
        #[cfg(test)]
        let offset = if use_test_rng {
            let mut test_rng = StepRng::new(1234567890u64, 1);
            BlindingFactor::from_secret_key(SecretKey::new(&keychain.secp(), &mut test_rng))
        } else {
            BlindingFactor::from_secret_key(SecretKey::new(&keychain.secp(), &mut rng()))
        };
        #[cfg(not(test))]
        let offset = {
            let _ = use_test_rng;
