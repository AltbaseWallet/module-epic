...
pub enum Error {
    ...
    RngError(String),
    /// Error converting recovery phrase to key material
    RecoveryPhraseError(mnemonic::Error),
}
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match *self {
            ...
            Error::RecoveryPhraseError(ref e) => fmt::Display::fmt(e, f),
        }
    }
}
...
impl ExtendedPrivKey {
    ...
    pub fn from_mnemonic(
        ...
        let seed = match mnemonic::to_seed(mnemonic, passphrase) {
            Ok(s) => s,
            Err(e) => return Err(Error::RecoveryPhraseError(e)),
        };
