    ...
    P: PoolAdapter + 'static,
{
    ...
    let tor_push_handler = TorAPIHandler::new(Arc::downgrade(&tx_pool));
    ...
    P: PoolAdapter,
{
    ...
    pub fn new(tx_pool: Weak<RwLock<pool::TransactionPool<B, P>>>) -> Self {
        TorAPIHandler { tx_pool }
    }
}
...
{
    fn post(&self, req: Request<hyper::body::Incoming>) -> ResponseFuture {
        let api = Tor::new(self.tx_pool.clone());
