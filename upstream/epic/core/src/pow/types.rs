...
impl PoWType {
    ...
    pub fn to_str(&self) -> String {
        match self {
            ...
            PoWType::RandomX => "algo002".to_string(),
            PoWType::ProgPow => "algo003".to_string(),
        }
    }
}
...
/// Common interface for proof-of-work verification.
pub trait PoWContext<T>
where
    T: EdgeType,
{
    /// Sets the header along with an optional nonce at the end
    /// The final flag is retained for wire-compatible verifier contexts.
    fn set_header_nonce(
        &mut self,
        header: Vec<u8>,
        nonce: Option<u64>,
