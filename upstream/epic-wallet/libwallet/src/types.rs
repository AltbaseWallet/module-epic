...
pub trait NodeClient: Send + Sync + Clone {
    ...
    /// Get outputs created in a block-height range. Implementations that can
    /// return spent outputs should set the final tuple flag to true for them.
    ...
    > {
        Err(Error::ClientCallback(
            "historical output scanning is not supported by this node client".to_string(),
        ))
    }
...
impl Context {
    ...
    pub fn new(
        ...
        #[cfg(test)]
        let sec_nonce = if use_test_rng {
            SecretKey::from_slice(secp, &[1; 32]).unwrap()
        } else {
            aggsig::create_secnonce(secp).unwrap()
        };
        #[cfg(not(test))]
        let sec_nonce = {
            let _ = use_test_rng;
