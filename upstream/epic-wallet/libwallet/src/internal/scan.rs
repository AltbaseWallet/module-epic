    ...
    K: Keychain + 'a,
{
    ...
    for output in outputs.iter() {
        let (commit, proof, is_coinbase, height, mmr_index, spent) = output;
...
fn historical_scan_chunk_size() -> u64 {
    env::var("EPIC_HISTORICAL_SCAN_CHUNK")
        .ok()
...
/// Collect all outputs created in a block-height range when the node client
/// supports block history. This recovers already-spent incoming outputs after
/// restoring from seed, where a plain UTXO scan would see nothing.
fn collect_chain_outputs_by_height<'a, C, K>(
    keychain: &K,
    client: C,
    start_height: u64,
    end_height: u64,
    status_send_channel: &Option<Sender<StatusMessage>>,
) -> Result<Option<Vec<OutputResult>>, Error>
where
    C: NodeClient + 'a,
    K: Keychain + 'a,
{
    let mut result_vec: Vec<OutputResult> = vec![];
    let chunk_size = historical_scan_chunk_size();
    ...
    while current <= end_height {
        let chunk_end = cmp::min(
            current.saturating_add(chunk_size).saturating_sub(1),
            end_height,
        );
        let percentage_complete = if total == 0 {
            0
        } else {
            cmp::min(
                ((chunk_end.saturating_sub(start_height).saturating_add(1) as f64 / total as f64)
                    * 100.0) as u8,
                99,
            )
        };
        if let Some(ref s) = status_send_channel {
            let _ = s.send(StatusMessage::Scanning(
                format!("Checking block outputs from {} to {}.", current, chunk_end),
                percentage_complete,
            ));
        }
        attempted = true;
        let outputs = match client.get_outputs_by_height_range(current, chunk_end) {
            Ok(outputs) => outputs,
            Err(e) => {
                let error_text = format!("{}", e);
                if result_vec.is_empty() && error_text.contains("not supported") {
                    return Ok(None);
                }
                return Err(e);
            }
    ...
    K: Keychain + 'a,
{
    ...
    loop {
        ...
        let outputs: Vec<(
            ...
            .into_iter()
            .map(|(commit, proof, is_coinbase, height, mmr_index)| {
                (commit, proof, is_coinbase, height, mmr_index, false)
            })
            .collect();
    ...
    K: Keychain + 'a,
{
    ...
    let log_id = if tx_stats.is_none() || output.is_coinbase {
        ...
        t.confirmation_height = Some(output.height);
    ...
    let _ = batch.save(OutputData {
        ...
        mmr_index: if output.mmr_index == 0 {
            None
        } else {
            Some(output.mmr_index)
        },
        commit,
        value: output.value,
        status: if output.spent {
            OutputStatus::Spent
        } else {
    ...
    K: Keychain + 'a,
{
    ...
    let (chain_outs, last_index) = match collect_chain_outputs_by_height(
        &keychain,
        client.clone(),
        start_height,
        end_height,
        ...
        Some(chain_outs) => (chain_outs, pmmr_range.1),
        None => collect_chain_outputs(
            &keychain,
            client,
            pmmr_range.0,
            Some(pmmr_range.1),
