    ...
    K: Keychain + 'a,
{
    ...
    fn get_mnemonic(
        ...
        let data_dir_name = data_dir_name.to_str().unwrap();
        let wallet_seed = WalletSeed::from_file(&data_dir_name, password)
            .map_err(|_| Error::Lifecycle("Error opening wallet storage file".into()))?;
        let res = wallet_seed
            .to_mnemonic()
    ...
    fn validate_mnemonic(&self, mnemonic: ZeroingString) -> Result<(), Error> {
        match WalletSeed::from_mnemonic(mnemonic) {
            Ok(_) => Ok(()),
            Err(_) => Err(Error::GenericError("Validating recovery phrase".into()))?,
        }
    }
    ...
    fn recover_from_mnemonic(
        ...
        WalletSeed::recover_from_phrase(data_dir_name, mnemonic, password)
            .map_err(|_| Error::Lifecycle("Error recovering from recovery phrase".into()))?;
        Ok(())
    }
    ...
    fn change_password(
        ...
        let orig_mnemonic = orig_wallet_seed
            .to_mnemonic()
            .map_err(|_| Error::Lifecycle("Error recovering recovery phrase".into()))?;

        // Back up existing seed, and keep track of filename as we're deleting it
        // once the password change is confirmed
        let backup_name = WalletSeed::backup_seed(data_dir_name).map_err(|_| {
            Error::Lifecycle("Error temporarily backing up existing wallet material".into())
        })?;

        // Delete seed file
        WalletSeed::delete_seed_file(data_dir_name).map_err(|_| {
            Error::Lifecycle("Unable to delete wallet storage file for password change".into())
        })?;
        ...
        info!("Wallet storage file created");
        ...
        if orig_wallet_seed != new_wallet_seed {
            let msg = format!(
                "New and old wallet material are not equal on password change, not removing backups."
            );
            return Err(Error::Lifecycle(msg).into());
        }
        // Removin
        if remove_backup {
            info!("Password change confirmed, removing backup wallet storage file.");
