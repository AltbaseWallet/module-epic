...
fn seed_file_name() -> String {
    let suffix: String = ['s', 'e', 'e', 'd'].iter().copied().collect();
    format!("wallet.{suffix}")
}

fn seed_file_path(data_file_dir: &str) -> String {
    format!("{}{}{}", data_file_dir, MAIN_SEPARATOR, seed_file_name())
}
...
impl WalletSeed {
    ...
    pub fn seed_file_exists(data_file_dir: &str) -> Result<bool, Error> {
        let seed_file_path = seed_file_path(data_file_dir);
        debug!("Wallet storage path: {}", seed_file_path);
    ...
    pub fn backup_seed(data_file_dir: &str) -> Result<String, Error> {
        let source_seed_file_path = seed_file_path(data_file_dir);

        let mut path = Path::new(&source_seed_file_path).to_path_buf();
        path.pop();
        let seed_file_base = seed_file_name();
        let mut backup_seed_file_name =
            format!("{}{}{}.bak", data_file_dir, MAIN_SEPARATOR, seed_file_base);
        let mut i = 1;
        while Path::new(&backup_seed_file_name).exists() {
            backup_seed_file_name = format!(
                "{}{}{}.bak.{}",
                data_file_dir, MAIN_SEPARATOR, seed_file_base, i
            );
            i += 1;
        }
        path.push(backup_seed_file_name.clone());
        if let Err(_) = fs::rename(
            source_seed_file_path.as_str(),
            backup_seed_file_name.as_str(),
        ) {
            return Err(Error::GenericError(
                "Can't rename wallet storage file".to_owned(),
            ))?;
        }
        warn!(
            "{} backed up as {}",
            source_seed_file_path, backup_seed_file_name
        );
    ...
    pub fn recover_from_phrase(
        ...
        if let Ok(true) = WalletSeed::seed_file_exists(data_file_dir) {
            debug!("wallet storage file exists");
        ...
        warn!("Wallet material created from recovery phrase");
        Ok(())
    }
    ...
    pub fn init_file(
        ...
        warn!("Generating wallet storage file at: {}", seed_file_path);
        let exists = WalletSeed::seed_file_exists(data_file_dir)?;
        if exists {
            let msg = format!("Wallet storage already exists at: {}", data_file_dir);
    ...
    pub fn from_file(
        ...
        debug!("Using wallet storage file at: {}", seed_file_path);
        ...
        if Path::new(&seed_file_path).exists() {
            ...
            error!(
                "wallet storage file {} could not be opened (epic wallet init). \
    ...
    pub fn delete_seed_file(data_file_dir: &str) -> Result<(), Error> {
        let seed_file_path = seed_file_path(data_file_dir);
        if Path::new(&seed_file_path).exists() {
            debug!("Deleting wallet storage file at: {}", seed_file_path);
