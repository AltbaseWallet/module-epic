...
/// Narrow production sender path used by embedded wallets.
pub fn init_send_tx_for_altbase<'a, T: ?Sized, C, K>(
    w: &mut T,
    keychain_mask: Option<&SecretKey>,
    amount: u64,
    minimum_confirmations: u64,
    ...
    K: Keychain + 'a,
{
    ...
    let message = message.map(|mut value| {
        value.truncate(USER_MESSAGE_MAX_LEN);
        value
    });
    let mut slate = tx::new_tx_slate(&mut *w, amount, 2, false, None)?;
...
/// Estimate a sender fee without compiling the generic sender dispatch path.
pub fn estimate_send_tx_for_altbase<'a, T: ?Sized, C, K>(
    w: &mut T,
    keychain_mask: Option<&SecretKey>,
    amount: u64,
    minimum_confirmations: u64,
