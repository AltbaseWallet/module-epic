...
use crate::types::{
    NodeClient, NodeStatus, OutputData, OutputStatus, TxLogEntry, TxLogEntryType, WalletBackend,
    WalletInfo,
};
use crate::{BlockFees, CbData, OutputCommitMapping};

fn sync_info_height(status: &NodeStatus, key: &str) -> Option<u64> {
    status.sync_info.as_ref()?.get(key).and_then(|value| {
        value
            .as_u64()
...
fn node_is_synced_enough(status: &NodeStatus) -> bool {
    if status.sync_status == "no_sync" {
        return true;
    }

    let current_height = sync_info_height(status, "current_height").unwrap_or(status.tip.height);
    let highest_height = sync_info_height(status, "highest_height").unwrap_or(status.tip.height);
    let tip_height = status.tip.height;

    matches!(status.sync_status.as_str(), "body_sync" | "header_sync")
        && current_height.saturating_add(2) >= highest_height
        && tip_height.saturating_add(2) >= highest_height
}
    ...
    K: Keychain + 'a,
{
    ...
    if !node_is_synced_enough(&node_status) {
        warn!(
