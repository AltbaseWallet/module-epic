...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match *self {
            ...
            Error::InvalidLength(ell) => write!(f, "invalid phrase/entropy length {}", ell),
        }
    }
}
    ...
    Option<&'a str>: From<T>,
{
    ...
    let prefix: String = ['m', 'n', 'e', 'm', 'o', 'n', 'i', 'c']
        .iter()
        .copied()
        .collect();
    let salt = (prefix + Option::from(passphrase).unwrap_or("")).into_bytes();
